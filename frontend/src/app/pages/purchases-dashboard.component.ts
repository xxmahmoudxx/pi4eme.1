          ...
          <table class="table">
            ...
            <tbody>
              <tr *ngFor="let row of purchases">
                ...
                <td>
                  <button class="btn-del" (click)="deletePurchase(row._id)" title="Delete">🗑️</button>
                </td>
              </tr>
            </tbody>
          </table>
...
export class PurchasesDashboardComponent implements OnInit {
  ...
  form: any = {
    ...
    supplier: '', item: '', category: '', quantity: 1, unitCost: 0, notes: '',
  };
  ...
  loadAll() {
    this.loading = true;
    this.api.getPurchaseKpis().subscribe({
  ...
  loadCharts() {
    this.api.getPurchasesOverTime('day').subscribe((data) => {
      this.lineChartData = {
        ...
        datasets: [{
          data: data.map((d) => d.total), label: 'Spending',
    ...
    this.api.getPurchasesBySupplier().subscribe((data) => {
      this.barChartData = {
        labels: data.map((d) => d._id),
        datasets: [{
          data: data.map((d) => d.total), label: 'Total',
  ...
  upload(file: File) {
    ...
    this.api.uploadPurchases(file).subscribe({
      next: (res: any) => {
        this.uploadMsg = `✅ Successfully imported ${res.imported} purchases`;
  ...
  addManual() {
    ...
    this.api.createPurchase(this.form).subscribe({
      next: () => {
        this.manualMsg = '✅ Purchase added successfully';
        this.form = { date: new Date().toISOString().slice(0, 10), supplier: '', item: '', category: '', quantity: 1, unitCost: 0, notes: '' };
        this.loadAll();
        this.manualLoading = false;
      },
      error: (err) => {
        this.manualMsg = err?.error?.message || 'Failed to add purchase';
